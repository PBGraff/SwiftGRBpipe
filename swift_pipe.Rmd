...
# Loading Libraries
```{r,cache=TRUE,warning=FALSE,load_library}
library(lattice)
library(ggplot2)
library(caret)
library(rattle)
...
```{r,cache=TRUE,warning=FALSE,rf_roc}
...
We plot a ROC to see performance varying over threshold value.
```{r,cache=TRUE,warning=FALSE,ada_roc}
...
We plot a ROC to see performance varying over threshold value.
```{r,cache=TRUE,warning=FALSE,svm_roc}
rocSVM <- calculateROC(testing$trigger_index, predSVM2[,2], d = 0.001)
plotROC(rocSVM, title = "Support Vector Machines ROC")
```

The optimal threshold is at a probability of `r rocSVM$pthresh[which.max(rocSVM$F1score)]` and achieves an accuracy of `r rocSVM$Accuracy[which.max(rocSVM$F1score)]*100`%.
